...
public class SignApiUse extends Constant {
    ...
    @OnGroup
    @Filter(value = "list")
    public void getList(GroupMsg groupMsg, MsgSender msgSender) {
        sign.signList(genShinService.selectUid("198424216"));
        CatCodeUtil util = CatCodeUtil.INSTANCE;
        msgSender.SENDER.sendGroupMsg(groupMsg, GenShinSign.getItemMsg() + "\n" + util.toCat("image", true, "file="
                + GenShinSign.getItemImg()));
    }
